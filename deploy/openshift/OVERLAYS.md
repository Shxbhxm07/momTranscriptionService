...
|---|---|---|
| Where Whisper + NeMo run | pods on OCP GPUs (`components/gpu-speech`) | the GB10 (`11.0.0.34`), over the LAN |
...
| GPU on OCP | 2 GPUs (or 1 with time-slicing/MIG), ~5 GB used | none |
| PVC for model files | `offline-mom-models`, ~3.2 GB | none |
