...
def _paths(msg: Dict[str, Any]) -> List[str]:
    """The documents to read, from `file_urls`. Blank entries are skipped.

    Unlike the audio service, `path` is NOT a fallback location here: this backend sends `path` as a
    field to hand back ("AsItIs"), and a prompt-only job would otherwise try to download "AsItIs".
    """
    return [u for u in (_get(msg, "file_urls", "fileUrls", default=[]) or []) if isinstance(u, str) and u.strip()]
